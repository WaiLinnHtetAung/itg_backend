...
@section('content')
    <div class="card-head-icon">
        ...
        <div>Edit {{ __('messages.blog.title') }}</div>
    </div>
    <div class="card mt-3 p-4 mt-3">
        <span class="mb-4">{{ __('messages.blog.title') }} Edition</span>
        <form action="{{ route('admin.blogs.update', $blog->id) }}" method="post" id="blog_edit" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                    ...
                    <div class="form-group mb-5">
                        ...
                        <select name="author_id" id="" class="form-control select2"
                            ...
                            @foreach ($users as $key => $value)
                                <option value="{{ $key }}"
                                    {{ old('author_id') || $blog->author_id == $key ? 'selected' : '' }}>
                                    {{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="col-lg-4 col-md-6 col-sm-12 col-12">
                    <div class="form-group mb-5">
                        ...
                        <input type="text" class="form-control date" name="date"
                            value="{{ old('date', $blog->date) }}" autocomplete="off" placeholder="YYYY-MM-DD">
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-sm-12 col-12">
                    <div class="form-group mb-3">
                        ...
                        <img src="{{ $blog->getUrl() }}" alt="" class="mt-3" width="150" id="imgPreview">
                    </div>
                </div>
                <div class="col-lg-8 col-md-6 col-sm-12 col-12">
                    <div class="form-group mb-3">
                        <label for="">{{ __('messages.blog.fields.content') }}</label>
                        <textarea name="content" id="" cols="30" rows="10" class="cke-editor">{{ old('content', $blog->content) }}</textarea>
                        @error('content')
...
@section('scripts')
    {!! JsValidator::formRequest('App\Http\Requests\Admin\UpdateBlogRequest', '#blog_edit') !!}
